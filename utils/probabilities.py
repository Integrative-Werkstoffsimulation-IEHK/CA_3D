...
class NucleationProbabilities:
    def __init__(self, param):
        ...
        self.hf = param["het_factor"]
        self.o_prime = 1 / self.hf_init
        self.het_factor_b = log(self.o_prime)
        self.const_a = (1 / (self.hf_init * self.nucleation_probability[0])) ** (-6 / 5)
        self.const_b = log(1 / (self.hf_init * self.nucleation_probability[0])) * (1 / 5)

    def set_constants(self, nucleation_probability, het_factor):
        self.nucleation_probability[:] = nucleation_probability
        self.hf = het_factor
        self.const_a = (1 / (self.hf * self.nucleation_probability[0])) ** (-6 / 5)
        self.const_b = log(1 / (self.hf * self.nucleation_probability[0])) * (1 / 5)
    ...
    def evolve(self, iteration):
        self.hf = self.hf_init * self.const_e ** (self.het_factor_b * iteration)
