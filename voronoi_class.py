...
class Microstructure:
    ...
    def voronoi_3d_cells(self, n_cells_per_axis, number_of_grains, seeds=None):
        ...
        for cell_ind, cell in enumerate(vor):
            for face in cell['faces']:
                ...
                if face['adjacent_cell'] not in ind_done_cells and face['adjacent_cell'] >= 0:
                    ...
                    zero_s = list(np.around(face_vertices_coordinates[0], decimals=5))
                    ...
                    for i, coordinate in enumerate(face_vertices_coordinates):
                        if i > 0:
                            ...
                            previous = list(np.around(face_vertices_coordinates[i - 1], decimals=5))
                            ...
                            while position < path_length:
                                ...
                                path_start_round = list(np.around(path_start, decimals=5))
                                position += step
        ...
        #
        self.grain_boundaries[ca_edges[0], ca_edges[1], ca_edges[2]] = True
        self.grain_boundaries[ca_faces[0], ca_faces[1], ca_faces[2]] = True
    ...
    def three(self, face_vert_coords):
        ...
        zero_s = list(np.around(face_vert_coords[0], decimals=5))
        # adding face's edges and some diagonals to divide it on triangles
        for i, coordinate in enumerate(face_vert_coords):
            if i > 0:
                # edg
                current = list(np.around(coordinate, decimals=5))
                previous = list(np.around(face_vert_coords[i - 1], decimals=5))
                if [current, previous] not in self.lines and [previous, current] not in self.lines:
                    self.lines.append([current, previous])
                if i > 1:
                    # diagonal (or closing line in case of the last edge)
                    if [current, zero_s] not in self.lines and [zero_s, current] not in self.lines:
                        if i == len(face_vert_coords) - 1:
                            self.lines.append([current, zero_s])
                        else:
                            self.lines_faces.append([current, zero_s])
                # filling triangle with lines
                step = 1 / self.n_cells_per_axis
                position = 0
                path_start = np.array(current)
                path_end = np.array(previous)
                path_length = np.linalg.norm(path_end - path_start)
                ...
                while position < path_length:
                    ...
                    path_start_round = list(np.around(path_start, decimals=5))
                    position += step
                    self.lines_faces.append([zero_s, path_start_round])
    ...
    def fill_plane(self, start_line_a, start_line_b, end_line_a, end_line_b):
        ...
        # step = 1 / self.divisor
        # position = 0
        # start_line_a = np.array(start_line_a)
        # end_line_a = np.array(end_line_a)
        #
        # start_line_b = np.array(start_line_b)
        # end_line_b = np.array(end_line_b)
        #
        # line_length_start = np.linalg.norm(end_line_a - start_line_a)
        # line_length_end = np.linalg.norm(end_line_b - start_line_b)
        #
        # vector_step_start = (end_line_a - start_line_a) * step / line_length_start
        # vector_step_end = (end_line_b - start_line_b) * step / line_length_end
        #
        # while position < path_length:
        #     path_start += vector_step
        #     path_start_round = list(np.around(path_start, decimals=5))
