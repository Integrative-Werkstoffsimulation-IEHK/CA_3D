...
class VoronoiMicrostructure:
    ...
    def generate_voronoi_3d(self, n_cells_per_axis, number_of_grains, periodic=False, seeds=None):
        self.divisor = int(n_cells_per_axis / 1)
        if seeds is None:
        ...
        # vor = pyvoro.compute_voronoi(seeds, [[0.0, 1.0], [0.0, 1.0], [0.0, 1.0]], 0.01, radii=[1.3, 1.4], periodic=[True, True, False])
        ...
        # cells_faces_3d = np.array(np.nonzero(ca_shifted_faces))
        # cells_edges_3d = np.array(np.nonzero(ca_shifted_edges))
    ...
    def generate_voronoi_3d_continious(self, n_cells_per_axis, number_of_grains, seeds=None):
        if seeds is None:
            seeds = np.random.random_sample((number_of_grains, 3))
            # print(seeds)
        # regular arrangement of seeds for some tests
        ...
        for start, end in zip(starts, ends):
            ...
            ax.plot([z_start, z_end], [y_start, y_end], [x_start, x_end], color=np.random.choice(colors))
