...
-----------------------------------------------------------------""")
        ...
        if self.param["save_whole"]:
            def animate_sep(iteration):
                ...
                if self.param["compute_precipitations"]:
                    ...
                    if np.any(items):
                        ax_precip.cla()
                        ax_precip.scatter(items[:, 2], items[:, 1], items[:, 0], marker=',', color='r', s=3)
                    if self.param["secondary_active_element_exists"]:
                        self.c.execute("SELECT * from secondary_product_iter_{}".format(iteration))
                        items = np.array(self.c.fetchall())
                        if np.any(items):
                            ax_sprecip.cla()
                            ax_sprecip.scatter(items[:, 2], items[:, 1], items[:, 0], marker=',', color='cyan', s=3)
            ...
            def animate(iteration):
                ...
                if self.param["compute_precipitations"]:
                    ...
                    items = np.array(self.c.fetchall())
                    if np.any(items):
                        ax_all.scatter(items[:, 2], items[:, 1], items[:, 0], marker=',', color='r', s=3)
                    if self.param["secondary_active_element_exists"]:
                        self.c.execute("SELECT * from secondary_product_iter_{}".format(iteration))
                        items = np.array(self.c.fetchall())
                        if np.any(items):
            ...
            fig = plt.figure()
            if animate_separate:
                ax_inward = fig.add_subplot(231, projection='3d')
                ax_sinward = fig.add_subplot(234, projection='3d')
                ax_outward = fig.add_subplot(232, projection='3d')
                ax_soutward = fig.add_subplot(235, projection='3d')
                ax_precip = fig.add_subplot(233, projection='3d')
                ax_sprecip = fig.add_subplot(236, projection='3d')
                animation = FuncAnimation(fig, animate_sep)
            else:
                ax_all = fig.add_subplot(111, projection='3d')
                animation = FuncAnimation(fig, animate)
            plt.show()
            # plt.savefig(f'W:/SIMCA/test_runs_data/{self.db_name}.jpeg')
        else:
        ...
        if plot_separate:
            ...
            if self.param["compute_precipitations"]:
                ...
                if np.any(items):
                    ...
                    # coords = np.unravel_index(counts[0], self.shape)
                    # counts = counts[1]
    ...
    def plot_2d(self, plot_separate=False, iteration=None, slice_pos=None):
        ...
        fig = plt.figure()
        if plot_separate:
            ax_inward = fig.add_subplot(231)
            ax_sinward = fig.add_subplot(234)
            ax_outward = fig.add_subplot(232)
            ax_soutward = fig.add_subplot(235)
            ax_precip = fig.add_subplot(233)
            ax_sprecip = fig.add_subplot(236)
            if self.param["inward_diffusion"]:
                self.c.execute("SELECT * from primary_oxidant_iter_{}".format(iteration))
                items = np.array(self.c.fetchall())
                if np.any(items):
                    ind = np.where(items[:, 0] == slice_pos)
                    ax_inward.scatter(items[ind, 2], items[ind, 1], marker=',', color='b', s=1)
                if self.param["secondary_oxidant_exists"]:
                    self.c.execute("SELECT * from secondary_oxidant_iter_{}".format(iteration))
                    items = np.array(self.c.fetchall())
                    if np.any(items):
                        ind = np.where(items[:, 0] == slice_pos)
                        ax_sinward.scatter(items[ind, 2], items[ind, 1], marker=',', color='deeppink', s=1)
            if self.param["outward_diffusion"]:
                self.c.execute("SELECT * from primary_active_iter_{}".format(iteration))
                items = np.array(self.c.fetchall())
                if np.any(items):
                    ind = np.where(items[:, 0] == slice_pos)
                    ax_outward.scatter(items[ind, 2], items[ind, 1], marker=',', color='g', s=3)
                if self.param["secondary_active_element_exists"]:
                    self.c.execute("SELECT * from secondary_active_iter_{}".format(iteration))
                    items = np.array(self.c.fetchall())
                    if np.any(items):
                        ind = np.where(items[:, 0] == slice_pos)
                        ax_soutward.scatter(items[ind, 2], items[ind, 1], marker=',', color='darkorange', s=3)
            if self.param["compute_precipitations"]:
                self.c.execute("SELECT * from primary_product_iter_{}".format(iteration))
                items = np.array(self.c.fetchall())
                if np.any(items):
                    ind = np.where(items[:, 0] == slice_pos)
                    ax_precip.scatter(items[ind, 2], items[ind, 1], marker=',', color='r', s=3)
                if self.param["secondary_active_element_exists"]:
                    self.c.execute("SELECT * from secondary_product_iter_{}".format(iteration))
                    items = np.array(self.c.fetchall())
                    if np.any(items):
                        ind = np.where(items[:, 0] == slice_pos)
                        ax_sprecip.scatter(items[ind, 2], items[ind, 1], marker=',', color='cyan', s=3)
            ...
            if self.param["compute_precipitations"]:
                ...
                if np.any(items):
                    ind = np.where(items[:, 0] == slice_pos)
                    ax_all.scatter(items[ind, 2], items[ind, 1], marker=',', color='r', s=3)
                if self.param["secondary_active_element_exists"]:
                    self.c.execute("SELECT * from secondary_product_iter_{}".format(iteration))
                    items = np.array(self.c.fetchall())
                    if np.any(items):
                        ind = np.where(items[:, 0] == slice_pos)
                        ax_all.scatter(items[ind, 2], items[ind, 1], marker=',', color='cyan', s=3)
    ...
    def animate_concentration(self, analytic_sol=False, conc_type="atomic"):
        def animate(iteration):
            ...
            matrix = (n_matrix_page - outward - primary_product - secondary_product - soutward)
            ...
            # matrix_moles = matrix * self.param["matrix_elem"]["moles_per_cell"] + \
            #                outward * self.param["active_element"]["primary"]["matrix_moles_per_cell"] + \
            #                soutward * self.param["active_element"]["secondary"]["matrix_moles_per_cell"]
            # matrix_mass = matrix * self.param["matrix_elem"]["mass_per_cell"] + \
            #               outward * self.param["active_element"]["primary"]["matrix_mass_per_cell"] + \
            #               soutward * self.param["active_element"]["secondary"]["matrix_mass_per_cell"]
            x = np.linspace(0, self.param["size"], self.axlim)

            if conc_type == "atomic":
                whole_moles = matrix_moles + inward_moles + primary_product_moles + outward_moles + \
                              secondary_product_moles + soutward_moles + sinward_moles
                ...
                whole_mass = matrix_mass + inward_mass + primary_product_mass + outward_mass + \
                             secondary_product_mass + soutward_mass + sinward_mass
    ...
    def plot_concentration(self, plot_separate=True, iteration=None, conc_type="atomic", analytic_sol=False):
        ...
        if plot_separate:
            ax1 = fig.add_subplot(121)
            ax2 = fig.add_subplot(122)
            # ax1.scatter(x, inward, s=10, color='b')
            # ax2.scatter(x, outward, s=10, color='g')
            # ax2.scatter(x, precipitations, s=10, color='r')
            ax1.plot(x, inward, color='b')
            ax2.plot(x, primary_product, color='r')
            ax2.plot(x, secondary_product, color='cyan')
            ax2.plot(x, outward, color='g')
            ...
            # ax.scatter(x, inward, s=5, color='b')
            # ax.scatter(x, outward, s=10, color='g')
            # ax.scatter(x, precipitations, s=10, color='r')
